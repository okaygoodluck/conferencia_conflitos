...
def _parse_date_range():
    ...
    if df and not di:
        return df, df
    di = _get_input("Data início (dd/mm/aaaa) [Deixe vazio para buscar Manobra/Sol]: ", "GDIS_DATA_INICIO")
    if not di:
        return "", ""
    df = _get_input("Data fim (dd/mm/aaaa) [Deixe vazio para o mesmo dia]: ", "GDIS_DATA_FIM")
    if not df:
        df = di
    return di, df
...
def _parse_base_manobra():
    return _get_input("Manobra base: ", "GDIS_MANOBRA_BASE")
...
def run_verificacao(base, data_inicio, data_fim, usuario, senha, progress_cb=None, situacoes=None, malhas=None, base_eq_manual=None, base_al_manual=None):
    ...
    if base and str(base).strip():
        ...
        print(f"[{time.strftime('%H:%M:%S')}] [INFO] Extraindo manobra base {base}...")
        ...
        if not data_inicio or data_inicio == "undefined":
            if b_ini: 
                ...
                print(f"[{time.strftime('%H:%M:%S')}] [INFO] Data início extraída da base: {data_inicio}")
            else:
                print(f"[{time.strftime('%H:%M:%S')}] [WARN] Não foi possível extrair a data de início da base.")

        if not data_fim or data_fim == "undefined":
            if b_fim:
                data_fim = b_fim.split()[0]
                print(f"[{time.strftime('%H:%M:%S')}] [INFO] Data fim extraída da base: {data_fim}")
            else:
                 print(f"[{time.strftime('%H:%M:%S')}] [WARN] Não foi possível extrair a data de término da base.")
    ...
    print(f"[{time.strftime('%H:%M:%S')}] [INFO] EQUIPAMENTOS NA BASE: {', '.join(sorted(beq)) if beq else 'Nenhum'}")
    print(f"[{time.strftime('%H:%M:%S')}] [INFO] ALIMENTADORES NA BASE: {', '.join(sorted(bal)) if bal else 'Nenhum'}")
    print(f"[{time.strftime('%H:%M:%S')}] [INFO] PERÍODO PESQUISADO: {data_inicio or 'NÃO DEFINIDO'} até {data_fim or 'NÃO DEFINIDO'}\n")
    ...
    if not data_inicio or not data_fim or data_inicio == "undefined" or data_fim == "undefined":
        print(f"[{time.strftime('%H:%M:%S')}] [ERROR] Datas de busca não definidas. Informe as datas manualmente ou verifique a manobra base.")
        return {
            "status": "erro",
    ...
    for numero in todos_unico:
        ...
        if eq_hit or al_hit:
            print(f"[{time.strftime('%H:%M:%S')}] [CONFLITO] Manobra {numero} possui itens em comum!")
...
def _parse_malhas_input():
    ...
    raw_input = _get_input("Digite as malhas (separadas por vírgula, deixe em branco para buscar em todas): ", "GDIS_MALHAS_PADRAO")
    if not raw_input:
...
def main():
    ...
    usuario = _get_input("Usuário: ", "GDIS_USUARIO")
    senha = _get_input("Senha: ", "GDIS_SENHA", is_password=True)
