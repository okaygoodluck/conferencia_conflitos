...
def _carregar_dados_equipamentos():
    ...
    if not os.path.exists(caminho_csv):
        print(f"⚠️  Aviso: Arquivo CSV não encontrado (Regras do Conferidor 7, 31 e 32 ignoradas):\n   {caminho_csv}")
        return dados
...
def main(manobra_param=None, usuario_param=None, senha_param=None, headless=False):
    ...
    manobra_num = manobra_param if manobra_param else _get_input("Digite o número da Manobra Base: ", "GDIS_MANOBRA_BASE")
    if not manobra_num:
        print("Número inválido.")
        return

    usuario = usuario_param if usuario_param else _get_input("Usuário: ", "GDIS_USUARIO")
    senha = senha_param if senha_param else _get_input("Senha: ", "GDIS_SENHA", is_password=True)
    ...
    with sync_playwright() as p:
        ...
        solicitacao_bloqueio_ra = page.evaluate("""() => {
            ...
            for (const l of labels) {
                ...
                if (t.includes('BLOQUEIO DE RA') || (t.includes('RA:')) || (t.includes('BLOQUEIO') && t.includes('RA'))) {
                    // 1. Tenta a célula vizinha imediata (padrão GDIS)
                    if (l.nextElementSibling) {
                        const nt = l.nextElementSibling.textContent.toUpperCase();
                        if (/\bSIM\b/.test(nt)) return "SIM";
                        if (/\bN[AÃ]O\b/.test(nt)) return "NAO";
                    }
                    
                    // 2. Se não achou na vizinha, olha a linha mas com regex restrito (âncora no rótulo)
                    const tr = l.closest('tr');
                    const rowText = (tr ? tr.textContent : l.parentElement.textContent).toUpperCase();
                    
                    // Regex: Captura SIM ou NAO apenas se vier logo após o rótulo RA
                    const strictMatch = rowText.match(/(?:BLOQUEIO DE RA|RA)[:\s\-\|]+(SIM|N[AÃ]O)\b/);
                    if (strictMatch) return strictMatch[1] === "SIM" ? "SIM" : "NAO";
                }
            }
            return null; // Força o fallback para olhar a Aba Manobra
        }""")
    ...
    if not manobra_param:
        if sys.stdin.isatty():
            ...
            print("\nExecução finalizada (modo não interativo).")
