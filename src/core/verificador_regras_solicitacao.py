...
def _carregar_dados_equipamentos():
    ...
    if not os.path.exists(caminho_csv):
        print(f"⚠️  Aviso: Arquivo CSV não encontrado (Regras 7, 31 e 32 ignoradas):\n   {caminho_csv}")
        return dados
    ...
    try:
        ...
        print(f"⚠️  Aviso: Erro ao carregar dados do CSV: {e}")
...
def main(manobra_param=None, usuario_param=None, senha_param=None, headless=False):
    print("=====================================================")
    print("      VERIFICADOR DE MANOBRAS (Regras 1 a 40)        ")
    print("=====================================================")
    ...
    regras_equipamentos = _obter_regras_equipamentos()
    dados_equipamentos = _carregar_dados_equipamentos()
    ...
    with sync_playwright() as p:
        ...
        browser = p.chromium.launch(executable_path=executavel, headless=headless if manobra_param else False)
        ...
        solicitacao_num = page.evaluate(f"""(manobra) => {{
            ...
            const headers = Array.from(tabela.querySelectorAll('thead th')).map(th => (th.innerText || '').toLowerCase());
                ...
                if (targets.includes(text)) {
                    ...
                    if (tr) {
                        ...
                        if (table) {
                            ...
                            if (parentTd && parentTd.nextElementSibling) {
                                ...
                                if (siblingTable) {
                                    ...
                                    if (rowIdx >= 0 && rowIdx < siblingRows.length) {
                        ...
                        if (cellTd) {
                            ...
                            if (cellIdx >= 0 && cellIdx + 1 < tr.children.length) {
                    ...
                    const rawCell = clean(cell.parentNode ? cell.parentNode.textContent : cell.textContent);
        ...
        manobra_dados = page.evaluate("""() => {
            ...
            const clean = (s) => {
                ...
                // Limpeza profunda de resíduos de scripts JSF (RichFaces/SimpleTogglePanel) e códigos de controle «»
                ...
                if (!etapaTextoHeader.trim()) {
                    ...
                    for (const cand of etapaCandidates) {
                        ...
                        if (c.includes('backgroundCinza') || txt.includes('Etapa:')) {
                            // Só avalia se o container for pequeno e se estiver antes da tabela atual
                            if (txt.length < 300 && (cand.compareDocumentPosition(tabela) & Node.DOCUMENT_POSITION_FOLLOWING)) {
                                ...
                                // EVITA SOBRESCRITA POR LINHAS ESPAÇADORAS VAZIAS!
                                if (candText.length >= 5 && candText.includes('Etapa')) {
                                    bestHeader = candText;
                    ...
                    etapaTextoHeader = bestHeader || 'ERRO_CANDIDATOS_VAZIOS_OU_INVISIVEIS';
                }
                ...
                for (const row of rows) {
                    ...
                    if (c.includes('backgroundCinza') || c.includes('ui-rowgroup-header') || c.includes('ui-widget-header') || textContent.includes('Etapa:')) {
                        if (textContent.length < 500) {
                            ...
                            // Se for cabeçalho de colunas (Nº Ação Eqpto), ignora para não poluir o nome da etapa
        ...
        manobra_etapas_headers = page.evaluate("""() => {
            const trs = Array.from(document.querySelectorAll('tr.backgroundCinza'));
            return trs.map(tr => {
                // Clona a linha para não mexer no DOM original
                const clone = tr.cloneNode(true);
                // Remove scripts, estilos e outros lixos que o textContent pegaria
                clone.querySelectorAll('script, style, input, button').forEach(el => el.remove());
                
                const tds = Array.from(clone.querySelectorAll('td'));
                const text = tds.map(td => td.textContent || '').join(' ');
                let dateStr = '';
                for (const td of tds) {
                    const match = (td.textContent || '').match(/\\d{2}\\/\\d{2}\\/\\d{4}\\s+\\d{2}:\\d{2}/);
                    if (match) {
                        dateStr = match[0];
                    }
                }
                return {
                    texto: text.replace(/[\\s\\xA0]+/g, ' ').trim(),
                    data_hora: dateStr
                };
            });
        }""")
        ...
        print("    Extraindo painel de 'Locais de Interrupção'...")
        page.evaluate("""() => {
        ...
        page.wait_for_timeout(4000) # Espera carregar AJAX
            ...
            for (const t of tables) {
                ...
                for (let i = 0; i < rows.length; i++) {
                    ...
                    for (let j = 0; j < cells.length; j++) {
                        const txt = cells[j].textContent.toLowerCase().trim();
                        if (txt === 'data de início' || txt === 'data de inicio') {
                            if (i + 1 < rows.length && rows[i+1].children.length > j) {
                                const val = clean(rows[i+1].children[j].textContent);
                                if (/\\d{2}\\/\\d{2}\\/\\d{4}/.test(val)) dIni = val;
                            }
                        }
                        if (txt === 'data de término' || txt === 'data de termino') {
                            if (i + 1 < rows.length && rows[i+1].children.length > j) {
                                const val = clean(rows[i+1].children[j].textContent);
                                if (/\\d{2}\\/\\d{2}\\/\\d{4}/.test(val)) dFim = val;
                            }
                        }
                    }
                }
        ...
        solicitacao_locais = page.evaluate("""() => {
            ...
            for (const tabela of tables) {
                ...
                for (let i = 0; i < rows.length; i++) {
                    ...
                    const tNum = texts.findIndex(t => t === 'número' || t === 'numero' || t === 'equipamento/trafo');
                    if (tNum >= 0) {
                        headerRowIdx = i;
                        idxNumero = tNum;
                        idxAlim = texts.findIndex(t => t.includes('alimentador'));
                        idxLocal = texts.findIndex(t => t === 'local' || t.includes('local'));
                        idxIni = texts.findIndex(t => t.includes('data') && (t.includes('inicio') || t.includes('início')));
                        idxFim = texts.findIndex(t => t.includes('data') && (t.includes('termino') || t.includes('término')));
                        break;
                    }
                }
    ...
    for item in solicitacao_locais:
        eq_norm = _norm_eqpto(item['numero'])
        obj = {
            'eq': eq_norm,
    ...
    print("\n>>> FASE 1: INTEGRIDADE VISUAL E SINTAXE")
    ...
    if falhas_r21:
        for f in falhas_r21:
            print(f"   ❌ REGRA 21: FALHA (Utilização do placeholder genérico 'AAA' detectada em {f}).")
    else:
        print("   ✅ REGRA 21: OK (Nenhum placeholder 'AAA' detectado).")
    ...
    if duplicatas_r28:
        for d in duplicatas_r28: print(f"   ❌ REGRA 28: FALHA ({d}).")
    elif manobra_dados:
        print("   ✅ REGRA 28: OK (Nenhuma macro duplicada para o mesmo equipamento na mesma etapa).")
    ...
    if manobra_etapas_headers:
        ...
        if falhas_r24:
            for f in falhas_r24: print(f"   ❌ REGRA 24: FALHA ({f} no cabeçalho da primeira etapa).")
        if alertas_r24:
            for a in alertas_r24: print(f"   ⚠️  REGRA 24: ALERTA ({a} no cabeçalho da primeira etapa).")
    ...
    if len(manobra_etapas_headers) >= 3:
        ...
        if len(datas_etapas) >= 2 and len(set(datas_etapas)) == 1:
            print(f"   ⚠️  REGRA 25: ALERTA (Todas as etapas possuem o mesmo horário ou horários incompatíveis: '{datas_etapas[0]}').")
        else:
            print("   ✅ REGRA 25: OK (Horários das etapas apresentam variação temporal).")
    ...
    if falhas_r20:
        print(f"   ❌ REGRA 20: FALHA (Exigem preenchimento do campo Observação \"Lamina ou Fusível?\": {', '.join(sorted(falhas_r20))}).")
    elif teve_macro_obs:
        print("   ✅ REGRA 20: OK (Macros MA63/MA77 com observação preenchida corretamente).")
    else:
        pass  # IGNORADA silenciosa

    print("\n>>> FASE 2: VALIDAÇÕES GLOBAIS (AUTORIZAÇÃO E PLANEJAMENTO)")
    ...
    for eq, items in manobra_map.items():
        # Captura os horários de todas as etapas do equipamento
        todos_horarios = []
        horarios_deslig = []
        ...
        for mi in items:
            ...
            if m_dt:
                ...
                if dt_obj:
                    todos_horarios.append(dt_obj)
                    if "DESLIGAMENTO" in etapa_full:
                        horarios_deslig.append(dt_obj)
        
        if not todos_horarios: continue
        
        # Início: Pega a primeira etapa de DESLIGAMENTO. Se não existir, o início não falha por antecipação.
        ini_man_real = min(horarios_deslig) if horarios_deslig else None
        # Término: Pega a última etapa de qualquer tipo (ou Religamento se quisermos ser específicos)
        fim_man = max(todos_horarios)
        ...
        falhas_r26 = []
        # Só valida antecipação se houve uma etapa marcada como Desligamento
        ...
        # Valida término sempre (última ação do eq na manobra)
        ...
        if falhas_r26:
            ...
            print(f"   ❌ REGRA 26: FALHA (Equipamento '{eq}': Horário da Manobra diverge com o da Solicitação. {msg_f}).")
        else:
            if not lim_ini or not lim_fim:
                print(f"   ❌ REGRA 26: FALHA (Equipamento '{eq}': Sem data autorizada na solicitação).")
            # Caso contrário, OK silencioso
    ...
    if solicitacao_bloqueio_ra == "SIM":
        ...
        if tem_macro_ra:
            print("   ✅ REGRA 5: OK (Planejamento da Manobra possui macro de bloqueio MA52/MA14/MA28).")
        else:
            print("   ❌ REGRA 5: FALHA (Exige RA na Solicitação, mas a Manobra NÃO possui macro MA52, MA14 ou MA28).")
    else:
        ...
        if macros_encontradas:
            ...
            print(f"   ❌ REGRA 5: FALHA (A Solicitação NÃO exige bloqueio de RA, mas a Manobra contém macros de bloqueio indevidas: {str_macros}).")
        else:
            print("   ✅ REGRA 5: OK (Não exige Bloqueio de RA e a Manobra não possui macros de bloqueio).")
    ...
    if exige_gerador:
        if manobra_etapas_headers and re.search(r'\bG[MB]T\s*:', manobra_etapas_headers[0]['texto'].upper()):
            print("   ✅ REGRA 23: OK (Uso de gerador justificado com GMT/GBT na primeira etapa).")
        else:
            print(f"   ⚠️  REGRA 23: ALERTA ({motivo_gerador}, mas a primeira etapa não declarou 'GMT:' ou 'GBT:').")
    else:
        print("   ✅ REGRA 23: OK (Nenhum uso de gerador detectado).")
    ...
    if falhas_r27:
        ...
        print("   ✅ REGRA 27: OK (Executores coerentes com os títulos das etapas).")
    ...
    if not sol_locais:
        ...
        for sol_item in sol_locais:
                    ...
                    if eq_digits:
                        for mi in manobra_dados:
                            ...
                            if eq_digits in obs_digits and len(eq_digits) >= 5:
                                ...
                                print(f"   ✅ REGRA 1: OK (Equipamento '{eq}' encontrado na coluna Observação da Manobra — provável Gerador/Vinculado).")
                                break  # sai do loop interno de manobra_dados
                
                if not encontrou_fallback:
                    print(f"   ❌ REGRA 1: FALHA (Equipamento da Solicitação '{eq}' NÃO ESTÁ na Manobra).")
                    continue
                
                if eq_em_observacao:
                    continue  # Pula Regras 3, 4, 12, 13... pois o eq não tem itens diretos na manobra
            
            if eq in manobra_map:
                print(f"   ✅ REGRA 1: OK (Equipamento '{eq}' está presente na Manobra).")
            ...
            if not sol_alim or sol_alim == "-":
                ...
                if alim_ok:
                    print("   ✅ REGRA 3: OK (Alimentador bate com a Manobra).")
                else:
                    alims_found = set(mi['alim'] for mi in manobra_items if mi['alim'])
                    alims_str = ", ".join(alims_found) if alims_found else "Nenhum"
                    print(f"   ❌ REGRA 3: FALHA (Equipamento '{eq}' com Alimentador divergente. Esperado: {sol_alim}, Encontrado: {alims_str}).")
    ...
    for eq, manobra_items in manobra_map.items():
        ...
        if prefixo and prefixo in regras_equipamentos and len(regras_equipamentos[prefixo]) > 0:
            acoes_proibidas = regras_equipamentos[prefixo]
            ...
            if acoes_encontradas_proibidas:
                ...
                print(f"   ❌ REGRA 6: FALHA (Equipamento '{eq}' possui ações incompatíveis: '{acoes_str}').")
            else:
                print(f"   ✅ REGRA 6: OK (Nenhuma ação incompatível no equipamento '{eq}').")
        else:
            pass  # IGNORADA silenciosa
        ...
        if eq in sol_dict:
            ...
            if prefixo == "02":
                ...
                if acao_ma64_encontrada:
                    print(f"   ✅ REGRA 7: OK (Equipamento telecontrolado '{eq}' possui macro MA64 - Modo Local).")
                else:
                    print(f"   ❌ REGRA 7: FALHA (Equipamento '{eq}' é telecontrolado, mas NÃO possui a macro MA64 na manobra).")
            else:
                print(f"   ✅ REGRA 7: OK (Equipamento '{eq}' não é telecontrolado, não exige MA64).")
        else:
            pass  # IGNORADA silenciosa
        ...
        if acoes_rt_encontradas:
            ...
            if not (is_rt or is_alim):
                ...
                print(f"   ❌ REGRA 8: FALHA (As ({str_macros}) so podem ser aplicadas em RT: '{eq}' não é RT).")
            else:
                print(f"   ✅ REGRA 8: OK (Macros exclusivas de RT aplicadas corretamente).")
        else:
            pass  # IGNORADA silenciosa
        ...
        if acoes_rd_encontradas:
            ...
            if not (is_relig_disj or is_alim):
                ...
                print(f"   ❌ REGRA 9: FALHA (Macros ({str_macros}) aplicadas para Religador/Disjuntor: '{eq}' inválido).")
            else:
                print(f"   ✅ REGRA 9: OK (Macros de Religador/Disjuntor aplicadas corretamente).")
        else:
            pass  # IGNORADA silenciosa
        ...
        if acoes_bloq_encontradas:
            ...
            if falha_regra10:
                print(f"   ❌ REGRA 10: FALHA (Macros ({str_macros_bloq}) invalida para '{eq}'. [Permitido apenas 01/04 / Alguns necessitam da Obs 'Chave Deslocada']).")
            else:
                print(f"   ✅ REGRA 10: OK (Macros de Bloqueio/Desbloqueio aplicadas corretamente).")
        else:
            pass  # IGNORADA silenciosa
        ...
        if acoes_ajustes_encontradas:
            ...
            if not (is_protecao or is_alim):
                ...
                print(f"   ❌ REGRA 11: FALHA (Macros de Alteração de Ajustes ({str_macros_ajustes}) aplicadas em equipamento inválido: '{eq}'). Permitido apenas para prefixos 21, 22, 23.")
            else:
                print(f"   ✅ REGRA 11: OK (Macros de Alteração de Ajustes aplicadas corretamente).")
        else:
            pass  # IGNORADA silenciosa
                ...
                if re.search(r'\b\d*MA01\b', mi['texto_linha'], re.IGNORECASE):
                    ...
                    if is_telecontrolado and not tem_mab9:
                        if not re.search(r'\bCORTE\s+DE\s+CARGA\b', mi['texto_linha'], re.IGNORECASE):
                            falha_r13 = True
        if falha_r13:
            print(f"   ⚠️  REGRA 13: ALERTA (Executor 'Região' executando MA01 sem 'CORTE DE CARGA'. A equipe está abrindo '{eq}' sem sinalizar!).")
        elif teve_ma01_regiao:
            if not is_telecontrolado or tem_mab9:
                print(f"   ✅ REGRA 13: OK (Equipamento manual ou justificado por MAB9. Não exige indicação explícita).")
            else:
                print(f"   ✅ REGRA 13: OK (Abertura local MA01 justificada com 'CORTE DE CARGA').")
        else:
            pass  # IGNORADA silenciosa
        ...
        for mi in manobra_items:
            ...
            if re.search(r'\bCOD\b', execut, re.IGNORECASE):
                ...
                if pos:
                    falha_r14 = True
        if falha_r14:
            print(f"   ❌ REGRA 14: FALHA (Equipamento '{eq}': Executor 'COD' não pode ter 'Posicionamento').")
        elif teve_cod:
            print(f"   ✅ REGRA 14: OK (Equipamento '{eq}': Operações do 'COD' estão sem Posicionamento).")
        else:
            pass  # IGNORADA silenciosa
        ...
        if falhas_r15:
            ...
            print(f"   ❌ REGRA 15: FALHA (Equipamento '{eq}': COD executando ({str_macros}) irregularmente. Motivo: {str_motivos}).")
        elif teve_operacao_cod:
            print(f"   ✅ REGRA 15: OK (Equipamento '{eq}': Operação do COD permitida e equipamento é telecontrolado).")
        else:
            pass  # IGNORADA silenciosa
        ...
        if falha_r16:
            ...
            print(f"   ❌ REGRA 16: FALHA (Equipamento '{eq}': Etapa 'VERIFICACAO PELO COD' possui executor inválido: '{str_executores}'). Apenas COD é aceito.")
        elif teve_verificacao_cod:
            print(f"   ✅ REGRA 16: OK (Equipamento '{eq}': Etapa 'VERIFICACAO PELO COD' executada corretamente pelo COD).")
        else:
            pass  # IGNORADA silenciosa
        ...
        if falha_r17:
            print(f"   ❌ REGRA 17: FALHA ({motivo_r17})")
        elif teve_ma09:
            print(f"   ✅ REGRA 17: OK (Macro MA09 aplicada corretamente).")
        else:
            pass  # IGNORADA silenciosa
        ...
        if falhas_r18:
            ...
            print(f"   ❌ REGRA 18: FALHA (Comandos de by-pass ({str_bp}) no equipamento '{eq}'. Permitido apenas em prefixos 02, 22, 23).")
        elif teve_bypass:
            print(f"   ✅ REGRA 18: OK (Comandos de by-pass aplicados em equipamento permitido).")
        else:
            pass  # IGNORADA silenciosa
        ...
        for mi in manobra_items:
            if re.search(r'\b\d*MAC1\b', mi['texto_linha'], re.IGNORECASE):
                ...
                if is_alim:
                    falha_r19 = True
        if falha_r19:
            print(f"   ❌ REGRA 19: FALHA (Macro MAC1 aplicada em Alimentador '{eq}'. Exige equipamento físico na rede).")
        elif teve_mac1:
            print(f"   ✅ REGRA 19: OK (Macro MAC1 aplicada corretamente em equipamento físico).")
        ...
        for mi in manobra_items:
            ...
            if posic == 'SIM':
                ...
                # Aproveitamos as macros_abertura/fechamento criadas na REGRA 31
                macros_abertura = re.compile(r'\b\d*(MA01|MA31|MA30|MA18|MA22|MA24|MA54|MA56|MAA9)\b(?!\s*-\s*OUTROS)')
                macros_fechamento = re.compile(r'\b\d*(MA02|MA66|MA67|MA19|MA23|MA25|MA55|MA57|MAB1)\b(?!\s*-\s*OUTROS)')
        ...
        if falhas_r39:
            ...
            print(f"   ❌ REGRA 39: FALHA (Equipamento '{eq}': Posicionamento marcado como 'Sim', mas violou as regras: {str_falhas}).")
        else:
            tem_pos_sim = any(mi.get('posicionamento', '').upper() == 'SIM' for mi in manobra_items)
            if tem_pos_sim:
                print(f"   ✅ REGRA 39: OK (Equipamento '{eq}': Posicionamento justificado com executor Região e ação de Abertura/Fechamento).")
            else:
                pass # IGNORADA silenciosa

    print("\n>>> FASE 4.1: RELAÇÕES DE EQUIPES")
    ...
    if manobra_dados and manobra_etapas_headers:
        ...
        if tem_executor_regiao and not tem_equipes_header:
            print("   ❌ REGRA 35: FALHA (Identificado item com executor 'Região', mas a diretriz 'EQUIPES:X' não foi informada ou falta a quantidade no cabeçalho da primeira etapa).")
        elif tem_equipes_header and not tem_executor_regiao:
            print("   ⚠️  REGRA 35: ALERTA (Informado 'EQUIPES:X' no cabeçalho, mas nenhum passo possui executor 'Região').")
        else:
            if tem_equipes_header and tem_executor_regiao:
                print("   ✅ REGRA 35: OK (Cabeçalho possui 'EQUIPES:X' e existe executor Região compatível).")
            else:
                pass # Ninguém tem Região nem Equipes: OK silencioso

    print("\n>>> FASE 5: BALANÇO E CRONOLOGIA (RAIO-X TEMPORAL)")
    if not manobra_map:
        print("⚠️  Manobra vazia. Sem equipamentos manobrados.")
    ...
    for eq, manobra_items in manobra_map.items():
        ...
        if falhas_r22_list:
            ...
            print(f"   ❌ REGRA 22: FALHA (Equipamento '{eq}': {str_falhas}).")
        elif teve_rastreio_inversa:
            print(f"   ✅ REGRA 22: OK (Equilíbrio de ações e cronologia de bloqueios validados em '{eq}').")
        else:
            pass  # IGNORADA silenciosa
        ...
        if falhas_r30:
            ...
            print(f"   ❌ REGRA 30: FALHA (Ordem cronológica invertida no equipamento '{eq}': {str_falhas}).")
        elif teve_acao_crono:
            print(f"   ✅ REGRA 30: OK (Ações e reversões executadas na ordem cronológica correta).")
        else:
            pass  # IGNORADA silenciosa

    # REGRA 32 (Incompatibilidade de Fases: Abrir Trifásico e Fechar Monofásico no MESMO ALIMENTADOR)
    print("\n🔹 Verificando Compatibilidade de Fases (Regra 32)...")
    falhas_r32 = False
    teve_fases = False
    ...
    for eh_grupo in grupos_etapas:
        ...
        for alim_key in set(abertos_por_alim.keys()) & set(fechados_por_alim.keys()):
            ...
            if abriu_tri and fechou_mono:
                ...
                print(f"   ❌ REGRA 32: FALHA (Etapa '{eh_grupo}' | Alim {alim_key}: Abrindo Trifásico ({str_tri}) e Fechando Monofásico ({str_mono}) na mesma etapa).")
            elif abriu_tri or fechou_mono:
                teve_fases = True
    
    if not falhas_r32 and teve_fases:
        print("   ✅ REGRA 32: OK (Equilíbrio de Fases validado nas transferências por alimentador).")
    elif not teve_fases and not falhas_r32:
        pass  # IGNORADA silenciosa

    # REGRA 33 (MA30 ASTA sem carga)
    print("\n🔹 Verificando Chave ASTA (Regra 33)...")
    falha_r33 = False
    for mi in manobra_dados:
        ...
        if re.search(r'\b\d*MA30\b', tx, re.IGNORECASE):
            ...
            if "COM CARGA" not in (tx + " " + ob):
                falha_r33 = True
                print(f"   ❌ REGRA 33: FALHA (Chave ASTA '{mi.get('equipamento')}' operada sem a indicação 'COM CARGA').")
    if not falha_r33 and any(re.search(r'\b\d*MA30\b', mi.get('texto_linha', ''), re.IGNORECASE) for mi in manobra_dados):
        print("   ✅ REGRA 33: OK (Todas as operações de ASTA possuem indicação de 'COM CARGA').")

    # REGRA 34 (MAB9 Compatibilidade de Prefixo)
    # Já validado pela REGRA 6 via _obter_regras_equipamentos()
    ...
    if 'num_equipes_header' in locals() and num_equipes_header > 0:
        ...
        if not tem_regiao:
            print(f"   ⚠️  REGRA 35: ALERTA (Cabeçalho indica EQUIPES:{num_equipes_header}, mas nenhuma ação possui 'Região' como executor).")
        else:
            print(f"   ✅ REGRA 35: OK (Equipes indicadas no cabeçalho e confirmadas por ações da Região).")

    # REGRA 36 (Sincronismo de Horário: Item vs Etapa)
    print("\n🔹 Verificando Sincronismo de Horário (Regra 36)...")
    falhas_r36 = []
    for mi in manobra_dados:
        # Extrai data/hora do cabeçalho da etapa
        eh_str = mi.get('etapa_texto_header', '')
        m_dt_header = re.search(r'(\d{2}/\d{2}/\d{4}\s+\d{2}:\d{2})', eh_str)
        
        # Extrai data/hora do próprio item
        dt_item_str = mi.get('data_hora', '').strip()
        
        if m_dt_header and dt_item_str and dt_item_str != "-":
            # Se ambos existem, comparamos
            dt_header = m_dt_header.group(1)
            # Normaliza o item caso venha com data completa ou só hora
            if len(dt_item_str) <= 5: # Só hora (ex: 08:00)
    ...
    if falhas_r36:
        for f in falhas_r36: print(f"   ⚠️  REGRA 36: ALERTA ({f}).")
    else:
        print("   ✅ REGRA 36: OK (Horários dos itens sincronizados com os cabeçalhos das etapas).")

    # REGRA 37 (Executor MA60 deve ser COD)
    print("\n🔹 Verificando Executor MA60 (Regra 37)...")
    falhas_r37 = []
    for mi in manobra_dados:
    ...
    if falhas_r37:
        for f in falhas_r37: print(f"   ❌ REGRA 37: FALHA ({f}).")
    elif any(re.search(r'\b\d*MA60\b', (mi.get('acao_bruta','') + " " + mi.get('texto_linha','')).upper()) for mi in manobra_dados):
        print("   ✅ REGRA 37: OK (Todas as ações MA60 atribuídas ao COD).")
    else:
        print("   ✅ REGRA 37: OK (Nenhuma ação MA60 detectada).")

    # REGRA 38 (Validação de Equipamentos Manuais vs Executor COD)
    if 'num_equipes_header' in locals() and num_equipes_header > 0:
        print("\n🔹 Verificando Execução de Equipamentos Manuais (Regra 38)...")
        falhas_r38 = []
